-- Initialize PC Builder Database with Sample Data
...
INSERT INTO compatibility_rules (source_category_id, target_category_id, rule_type, condition, severity, description) VALUES
-- CPU-Motherboard compatibility
(1, 3, 'socket_match', '{"attribute": "socket_type", "operator": "equals"}', 'error', 'CPU socket must match motherboard socket'),
(3, 1, 'socket_match', '{"attribute": "socket_type", "operator": "equals"}', 'error', 'Motherboard socket must match CPU socket'),

-- Memory-Motherboard compatibility
(4, 3, 'memory_type', '{"attribute": "memory_type", "operator": "equals"}', 'error', 'Memory type must be supported by motherboard'),
(3, 4, 'memory_type', '{"attribute": "memory_type", "operator": "equals"}', 'error', 'Motherboard must support memory type'),

-- Power Supply compatibility
(6, 2, 'power_supply', '{"attribute": "power_rating", "operator": "gte", "multiplier": 1.2}', 'warning', 'Power supply should be at least 120% of total system power'),
(6, 1, 'power_supply', '{"attribute": "power_rating", "operator": "gte", "multiplier": 1.2}', 'warning', 'Power supply should be at least 120% of total system power'),

-- Case compatibility
(7, 3, 'form_factor', '{"attribute": "form_factor", "operator": "supports"}', 'error', 'Case must support motherboard form factor'),
(7, 2, 'gpu_length', '{"attribute": "max_gpu_length", "operator": "gte"}', 'error', 'Case must accommodate GPU length'),
(7, 8, 'cpu_cooler_height', '{"attribute": "max_cpu_cooler_height", "operator": "gte"}', 'error', 'Case must accommodate CPU cooler height');

-- Update component ratings based on reviews
UPDATE components SET rating = 4.7, review_count = 1250 WHERE name = 'Intel Core i9-13900K';
UPDATE components SET rating = 4.6, review_count = 890 WHERE name = 'Intel Core i7-13700K';
UPDATE components SET rating = 4.5, review_count = 756 WHERE name = 'Intel Core i5-13600K';
UPDATE components SET rating = 4.8, review_count = 1100 WHERE name = 'AMD Ryzen 9 7950X';
UPDATE components SET rating = 4.6, review_count = 920 WHERE name = 'AMD Ryzen 7 7700X';
UPDATE components SET rating = 4.4, review_count = 680 WHERE name = 'AMD Ryzen 5 7600X';

-- Insert some sample users (password hashes would be generated by Django)
INSERT INTO users (username, email, password_hash, first_name, last_name, is_pro_builder) VALUES
('admin', 'admin@pcbuilder.com', 'pbkdf2_sha256$600000$hash_here', 'Admin', 'User', TRUE),
('john_doe', 'john@example.com', 'pbkdf2_sha256$600000$hash_here', 'John', 'Doe', FALSE),
('jane_smith', 'jane@example.com', 'pbkdf2_sha256$600000$hash_here', 'Jane', 'Smith', TRUE);

-- Insert sample builds
INSERT INTO builds (user_id, name, description, total_price, build_type) VALUES
(2, 'Gaming Beast', 'High-end gaming PC for 4K gaming', 0, 'gaming'),
(3, 'Workstation Pro', 'Professional workstation for content creation', 0, 'workstation');

-- Insert build components
INSERT INTO build_components (build_id, component_id, quantity) VALUES
(1, 1, 1), -- i9-13900K
(1, 4, 1), -- RTX 4090
(1, 7, 1), -- ASUS ROG Maximus Z790 Hero
(1, 10, 1), -- Corsair Dominator Platinum RGB
(1, 13, 1), -- Samsung 990 Pro
(1, 16, 1), -- Corsair HX1200
(1, 19, 1), -- Fractal Design Torrent
(1, 22, 1), -- Noctua NH-D15

(2, 5, 1), -- Ryzen 9 7950X
(2, 8, 1), -- RX 7900 XTX
(2, 10, 1), -- ASUS ROG Crosshair X670E Hero
(2, 11, 1), -- G.Skill Trident Z5 RGB
(2, 14, 1), -- WD Black SN850X
(2, 17, 1), -- EVGA SuperNOVA 1000 GT
(2, 20, 1), -- NZXT H7 Flow
(2, 23, 1); -- be quiet! Dark Rock Pro 4

-- Calculate build totals
SELECT calculate_build_total_price(1);
SELECT calculate_build_total_price(2);
